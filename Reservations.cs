...
namespace Parking_Finder
{
    public partial class Reservations : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            DataTable d = controllerObj.SelectAllParking(-1, Convert.ToInt32(textBox2.Text));
            if (d != null)
            {
            ...
            if(diff <0)
            {
                DataTable ss = controllerObj.FindInParkingCars(spotid, Convert.ToInt16(t.Rows[0]["Lane"]), Convert.ToChar(t.Rows[0]["Section"]), Convert.ToInt16(t.Rows[0]["Floor"]));
                if (ss != null)
                {
                    new AddParkingCar(spotid, (int)comboBox2.SelectedValue).Show();
                    return;
                }
                arr=now;
            }
        ...
        private void Reservations_FormClosed_1(object sender, FormClosedEventArgs e)
        {
            controllerObj.TerminateConnection();
        }
    }
